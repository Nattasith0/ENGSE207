...
async function fetchTasks() {
    ...
    try {
        ...
        if (!response.ok) throw new Error("โหลด tasks ล้มเหลว");
        const result = await response.json();
        allTasks = result.data || [];
        renderTasks();
    } catch (error) {
        alert("ไม่สามารถโหลด tasks จาก server ได้");
        console.error(error);
    } finally {
        hideLoading();
    }
...
document.addEventListener("DOMContentLoaded", () => {
    console.log("Task board initialized (Client–Server).");
    fetchTasks();
});
